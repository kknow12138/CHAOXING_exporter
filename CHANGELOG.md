# 更新日志

## v1.4.0 · 2026-06-14（雨课堂导出增强）

- 新增雨课堂 AI 学习空间 `lms-graph/.../exercise/...` 作业页专用抓取路径：从当前作业节点解析课堂 ID 与作业 ID，通过 `leaf_info` 和 `get_exercise_list` 接口直接拉取题目。
- 新增雨课堂 `studentCards` 习题集页面支持：在 `yuketang.cn/v2/web/studentCards/...` 页面点击「导出」，可通过接口拉取全部题目并导出为 HTML 文件。
- 题目内容从 `ProblemBodys[0].Paragraphs → Lines → Texts → Text` 路径精确提取文字（非截图/缩略图），选项 A/B/C/D 文字同步提取并高亮正确答案。
- 支持单选题、多选题、填空题、主观题、投票题，题型自动识别。
- 生成的 HTML 内置「🖨️ 打印 / 导出 PDF」按钮，点击触发浏览器打印对话框，可直接另存为 PDF；打印时按钮自动隐藏，分页不截断题目。
- 修复在课程知识点目录页误抓整页文本的问题，导出时优先抓取当前“作业”标头对应的测试题内容。
- 修复雨课堂随机字体导致的导出乱码：读取接口返回的当前作业字体文件，基于内置字形哈希表动态生成解密映射。
- 修复二次解密问题：只解密 `.xuetangx-com-encrypted-font` 等加密节点，不再对普通中文全文替换，避免把正常文本映射成错字。
- 支持 AI 学习空间作业导出为 Word 或 HTML，保留题干、选项、正确答案、我的答案、解析与题目图片。
- 更新扩展说明、测试说明和弹窗样式；版本号同步提升到 `1.4.0`。
- 原有雨课堂试卷/考试结果页面的 Word 导出流程不受影响，多种导出模式自动区分。

## 2026-06-13（文档与发布）

- 补充 macOS / Linux 安装与配置说明（py_exporter/README）。
- 新增 GitHub Actions 工作流：推送 `v*` 标签自动打包 Windows exe 并发布到 Releases。
- 首页 README 更新日志同步新增 v1.3.0（Python 命令行版）。

## 2026-06-13（二维码登录优化）

- Python 版二维码登录改为：生成 PNG 图片并自动用系统默认看图工具打开，同时打印可复制的登录链接，解决控制台字符二维码扫不动的问题。
- 新增 Pillow 依赖；更新 Windows 打包参数，确保 qrcode/PIL 正确打进 exe。

## 2026-06-13（新增 Python 版）

- 新增 `py_exporter/`：独立的 Python 命令行导出工具，与浏览器扩展功能等价但走学习通接口。
- 支持二维码 + 账号密码两种登录（密码经 AES 加密发送，不落盘）。
- 抓取课程/作业列表并解析已完成作业题目，支持单选/多选/判断/填空/简答/论述/编程题。
- 支持导出为 Word(.docx) / JSON / Markdown 三种格式，交互式菜单选课程、作业、格式。
- 仅读取与导出，**不含任何自动作答或提交作业功能**；附离线解析器测试与使用说明。
- 更新根 README，新增「使用声明」与 Python 版使用说明。

## 2026-06-13（下午）

- 新增 `findChaoxingQuestionsByBlocks`：遍历 `.mark_item` 题型块逐块拆出单题。
- 新增 `getChaoxingBlockType`：从块头（如「三、填空题」）读取权威题型，替代易出错的文本推断，确保 52 道题题型 100% 准确。
- 重构 `findQuestions`：抽出 `findBestContainerSet`，在块级与全局两种拆题结果中取题量更多者。
- 重新打包 `service-worker.js`，纳入综合填空题 `optionGroups`（子题选项分组）导出。

## 2026-06-13

- 修复学习通填空题/材料题中内嵌选择题选项未被正确导出的问题。
- 新增填空题 `optionGroups` 解析：可按小问 `（1）`、`（2）` 等分组提取 A/B/C/D 选项。
- 优化 Word 导出：填空题现在会在【题目内容】后增加【选项】板块，按小问展示选项。
- 修复学习通填空题答案抓取：优先读取 `dl.mark_fill.colorGreen` 的正确答案和 `dl.mark_fill.colorDeep` 的我的答案，避免把得分混入答案。
- 已用学习通作业详情页面中的第 29、30 题验证：多组填空选项和长会计分录选项均可完整读取。
